...
export default function ProfileClient({ userId, initialUser }: ProfileClientProps) {
  const [fullUserInfo] = useState<IUser | null>(initialUser);
  ...
  const onSubmit = async (data: z.infer<typeof updateUserValidation>) => {
    ...
    try {
      await axios.post("/api/user/update-user", data);
      toast.success("User info updated successfully");
    } catch (error) {
      toast.error(handleApiError(error, "Error while submitting user info"));
    } finally {
  ...
  const handleShareProfile = () => {
    ...
    navigator.clipboard
      ...
      .catch(() => toast.info(`Profile URL: ${url}`));
  };
    ...
    <div className="w-full min-h-screen">
      ...
      <div className="w-full customBackground pb-8">
        <div className="w-full flex justify-center gap-8 items-start pt-8 px-8">
          <div className="w-[20%]">
            ...
            {navItems.map(({ key, label, icon }) => (
              <div
                ...
                className={`flex items-center gap-4 px-4 py-3 cursor-pointer ${activeTab === key ? "text-foreground" : "text-gray-500"}`}
              >
                {icon}
                <h3>{label}</h3>
          ...
          <div className="w-[60%] min-h-[28rem] customBackground rounded-md py-4 px-8 border relative overflow-hidden">
            {activeTab !== "basic" && (
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    ...
                    <FormItem className="flex gap-12">
                      ...
                      <FormControl>
                        <Input
                          ...
                          {...field}
                          className="text-base p-4 h-11 w-[60%]"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <div className="flex items-center gap-12 mb-4">
                  ...
                  <h2 className="w-[60%] dark:bg-[#212124] focus-visible:border-ring border border-input h-10 rounded-md px-4 py-2 cursor-not-allowed">
                    {fullUserInfo?.email}
                  </h2>
                </div>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex gap-12">
                      ...
                      <textarea
                        className="w-[60%] h-24 rounded-md resize-none dark:bg-[#212124] border border-input focus-visible:border-ring px-4 py-2"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex gap-12">
                      ...
                      <FormControl>
                        <Input
                          placeholder="Skills (add space seperated tags | Max 6)"
                          value={Array.isArray(field.value) ? field.value.join(" ") : field.value}
                          onChange={(e) => {
                            const allSkills = e.target.value.split(" ").map((ele) => ele.trim());
                ...
                <div className="w-full flex justify-center">
                  <div className="w-[60%] flex gap-2">
                    {allValues.skills.map((skill, index) => (
                      <h3 key={index} className="px-4 py-1 bg-[var(--sidebar-accent)] rounded-full">
                        {skill}
                      </h3>
                    ))}
                  </div>
                ...
                <div className="w-full flex justify-center mt-3">
                  <Button
                    ...
                    type="submit"
                    className="w-[60%] font-semibold cursor-pointer h-10 text-base"
                  >
                    {isSubmitting ? (
                      <>
                        <Loader2 className="resize-custom animate-spin w-7 h-7" /> Please wait
                      </>
                    ) : (
                      "Save"
                    )}
